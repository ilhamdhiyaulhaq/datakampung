    ...
    if(edit_user($_POST) > 0) {
        echo "<script>
			Swal.fire({title: 'Update Data Berhasil',text: '',icon: 'success',confirmButtonText: 'OK'
				...
				window.location = '?page=user_bendahara';
				}
			})</script>";
    } else {
        echo "<script>
    ...
    <form action="" method="post" >
        ...
        <input type="hidden" class="form-control" name="id" value="<?= $edit['id']; ?>">
            ...
            <div class="input-group mb-3">
              <input class="form-control" type="hidden" name="level" value="Bendahara" readonly>
            </div>

		</div>
		<div class="card-footer">
			<input type="submit" name="simpan" value="Simpan" class="btn btn-info">
			<a href="?page=user_bendahara" title="Kembali" class="btn btn-secondary">Batal</a>
		</div>
	</form>
</div>
